...
func TestEncryption(t *testing.T) {
	...
	msg := []byte("dette er en krypteret testpakke")
	var key = "hemmeligkode"

	r, err := http.NewRequest("PUT", "/api/krypteret.txt?key="+key, bytes.NewBuffer(msg))
	if err != nil {
		t.Fatal(err)
	}
	...
	r, err = http.NewRequest("GET", "/api/"+id+"?key="+"ikkedenrigtigekode", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Test download with correct key
	r, err = http.NewRequest("GET", "/api/"+id+"?key="+key, nil)
	if err != nil {
		t.Fatal(err)
	}
